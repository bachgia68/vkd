...
//
// Cách chạy (từ thư mục project) — site chính đang deploy trên Vercel, endpoint dưới /api/:
//   PAYOS_CLIENT_ID=xxx PAYOS_API_KEY=xxx PAYOS_CHECKSUM_KEY=xxx \
//     node scripts/confirm-payos-webhook.mjs https://tasamngoclinh.com/api/payos-webhook
...
if (!webhookUrl) {
  ...
  console.error('  node scripts/confirm-payos-webhook.mjs https://<site>/api/payos-webhook');
  process.exit(1);
}
