...
type Page = 'home' | 'catalog' | 'vkd-catalog' | 'vkd-detail' | 'research' | 'checkout' | 'order-success' | 'loyalty' | 'autoship';

function App() {
  const [lang, setLang] = useState<Language>('vi');
  const [currentPage, setCurrentPage] = useState<Page>('vkd-catalog');
    ...
    <CartProvider>
      <div className="min-h-screen bg-cream-50">
        ...
        <main>
          ...
          {currentPage === 'vkd-catalog' && (
            <VKDProductCatalog onNavigate={navigate} />
          )}

          {currentPage === 'vkd-detail' && (
            <VKDProductDetail />
          )}
