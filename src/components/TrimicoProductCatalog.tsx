...
import {
  ...
  type TrimicoProduct,
  type TrimicoCategory,
  type TrimicoCategoryId,
} from '../data/trimicoProducts';
import type { Language } from '../i18n/translations';
import { useCart } from '../context/CartContext';

/**
 * TrimicoProductCatalog
 * -----------------------------------------------------------------------------
 * Trang danh mục sản phẩm Danh Mục Bổ Sung — cùng phong cách "Quiet Luxury"
 * với danh mục VKD, dữ liệu thật đồng bộ từ trimico.vn ngày 2026-08-02.
 * Danh mục rượu gắn badge "Chỉ trưng bày — chờ cấp phép, 18+" và ẩn nút đặt hàng
 * trực tiếp theo đúng thông báo trên trang gốc.
 * -----------------------------------------------------------------------------
 */

const categoryIcons: Record<TrimicoCategoryId, typeof Leaf> = {
  'sam-ngoc-linh': Leaf,
  'nam-lim-xanh': FlaskConical,
  ruou: Wine,
  khac: Sparkles,
  'qua-tang': Gift,
};

export default function TrimicoProductCatalog({
  onNavigate,
}: {
  lang: Language;
  onNavigate: (page: string, slug?: string) => void;
}) {
  const [activeCategory, setActiveCategory] = useState<TrimicoCategoryId | 'all'>('all');
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState<'default' | 'price-asc' | 'price-desc'>('default');
  const [mobileFiltersOpen, setMobileFiltersOpen] = useState(false);

  const filtered = useMemo(() => {
    let list = trimicoProducts.filter((p) => {
  ...
  const countByCategory = (id: TrimicoCategoryId) =>
    trimicoProducts.filter((p) => p.category === id).length;
      ...
      <div className="container-wide" style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        <div className="text-center max-w-3xl mx-auto mb-14">
          ...
          <p className="text-forest-600 text-lg leading-relaxed">
            Sâm Ngọc Linh, Nấm Lim Xanh, mật ong rừng và đặc sản Quảng Nam — đặt hàng và thanh toán trực tiếp trên TA. Danh mục rượu chỉ trưng bày,
            chờ xác nhận Bộ Công Thương, không bán cho người dưới 18 tuổi.
          </p>
        </div>
            ...
            <div className="sticky top-6 space-y-6">
                ...
                <ul className="space-y-1.5">
                  <li>
                    <CategoryButton
                      ...
                      count={trimicoProducts.length}
                    />
                  </li>
                  {trimicoCategories.map((cat) => {
              ...
              <div className="rounded-2xl bg-forest-900 p-5 text-cream-100 shadow-elegant">
                ...
                <p className="text-sm leading-relaxed text-cream-200">
                  Thông tin, hình ảnh và giá bán đồng bộ trực tiếp từ TRIMICO. Đặt hàng, giỏ hàng và
                  thanh toán xử lý trọn gói trên TA.
                </p>
              </div>

              <div className="rounded-2xl bg-gold-50 border border-gold-200 p-5">
                <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs leading-relaxed text-forest-600">
                  Sản phẩm rượu chỉ trưng bày, đang chờ xác nhận của Bộ Công Thương. Không bán cho
              ...
              <p className="text-sm text-forest-500">
                ...
                {activeCategory !== 'all' && (
                  <span className="text-forest-400">
                    {' · '}
                    {trimicoCategories.find((c) => c.id === activeCategory)?.label}
                  </span>
                )}
              </p>
              {query && (
...
function ProductCard({
  ...
  product: TrimicoProduct;
  category?: TrimicoCategory;
  onNavigate: (page: string, slug?: string) => void;
}) {
  ...
  return (
    <div className="group flex flex-col bg-white rounded-2xl overflow-hidden border border-cream-200 hover:border-gold-300 transition-all duration-500 hover:shadow-elegant-lg hover:-translate-y-1">
      ...
      <div className="p-5 flex flex-col flex-1">
        <button onClick={() => onNavigate('trimico-product-detail', product.slug)} className="text-left">
        ...
        <div className="mt-auto pt-4 border-t border-cream-200 flex items-center justify-between gap-3">
          <div>
            <div className="text-lg font-display font-bold text-forest-900">{formatVNDorContact(product.price)}</div>
            <div className="text-[11px] text-forest-400">Giá bán lẻ · VND</div>
          </div>
          {canOrder ? (
            <button
              onClick={() => addToCart(toTrimicoCartProduct(product))}
